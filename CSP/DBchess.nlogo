...
figures-own [ domain possible-steps step-performed? board-position ]
...
;; Ініціалізація налаштувань фігур, дошки
;; RPUBS(https://rpubs.com/ignat/436470)
;;
...
to draw-board
  foreach all-positions [[i] ->
    let x item 0 i
    let y item 1 i
    ask patch x y [ifelse (((x mod 2) = 0) xor ((y mod 2) = 0)) [set pcolor 8][set pcolor brown]]
  ]
end
...
to setup
  ...
  ask edges [
    set weight 1 ;; початкова вага ребер = 1
    set label weight
  ]
  ask figures [
    set label who
...
;; Призначити клітиці дошки певну фігуру
to move-to-cell [a]
  setxy (item 0 a) (item 1 a)
end

;; Призначити початкове розташування фігур
to assign-figures
  reset-ticks

  ask figures [
    set board-position one-of domain
    while [count turtles-on patch (item 0 board-position) (item 1 board-position) >= 1][
      set board-position one-of domain
    ]
    move-to-cell board-position
  ]
end

;; Підрахувати конфліктні зв'язки
to-report bad-links-count
   report count links with [
    ([board-position] of end1) = ([board-position] of end2)
    or
    member? ([board-position]) of end1 (attack-positions ([shape] of end2) ([board-position] of end2))
    or
    member? ([board-position]) of end2 (attack-positions ([shape] of end1) ([board-position] of end1))
]
end

to-report violations?
  report bad-links-count != 0
end
...
;; Зміна розташувань фігур на дошці
;; Імплементація алгоритму Distributed Breackout
;; RPUBS(https://rpubs.com/ignat/434313)
...
;; Список можливих розташувань
to-report conflict-links [the-assignment]
   report my-links with [
    ([board-position] of end1) = ([board-position] of end2)
    or
    (end1 != myself and member? the-assignment (attack-positions ([shape] of end1) ([board-position] of end1)))
    or
    (end2 != myself and member? the-assignment (attack-positions ([shape] of end2) ([board-position] of end2)))
  ]
end

;; Сума ціни розташувань
to-report cost-of-assignment [the-assignment]
  report sum ([weight] of conflict-links the-assignment)
end

;; Пошук виграшних розташувань фігур на дошці
to-report best-gain-assignment
  let assignment-costs map [[i] -> (list i cost-of-assignment i)] all-positions
  let min-cost min get-all-values assignment-costs
  let best-assignments get-all-keys-for-value assignment-costs min-cost
  let current-cost (get-value assignment-costs board-position)
  let gain (current-cost - min-cost)
  report (list gain (one-of best-assignments))
end

;; Зміна розташувань фігур на дошці
to change-assignment
  let gain-assignment best-gain-assignment ; список з виграшних розташувань
  let my-gain (first gain-assignment)
  let max-neighbors-gain max get-all-keys ([best-gain-assignment] of link-neighbors)
  if (my-gain >= max-neighbors-gain) [
    ask (conflict-links board-position)[
      set weight weight + 1
      set label weight
    ]
   set board-position (item 1 gain-assignment)
    move-to-cell board-position
  ]
end
...
;; Реалізація перевірки умов атак
;; Ферзя (queen) та Коня (knight)
;;
to-report knight-attack-positions [current-position]
  let x ( item 0 current-position)
  let y ( item 1 current-position)
  let ap (list
    (list (x - 1) (y + 2))
    (list (x - 2) (y + 1))
    (list (x - 1) (y - 2))
  ...
  report filter [attack-pos -> member? attack-pos all-positions] ap
end

to-report queen-attack-positions [current-position]
  let x ( item 0 current-position)
  let y ( item 1 current-position)
  let ap []
...
to-report attack-positions [figureShape current-position]
  if (figureShape = "chess knight") [
    report knight-attack-positions current-position
  ]
  if (figureShape = "chess queen") [
    report queen-attack-positions current-position
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
